...
func comm0(cir string,cID int, chVDCSCommCircRes chan<- int) {fmt.Println(cir)
fmt.Println(cID)
...
func comm1(cir string,cID int, chVDCSCommCircRes chan<- int) {fmt.Println(cir)
fmt.Println(cID)
//get the circuit in JSON format
//Generate input wires
//post to server
...
func eval1(s3 string,s2 string,cID int, chVDCSEvalCircRes <-chan int) (bool){
_inWire0:=[]byte(s3)
